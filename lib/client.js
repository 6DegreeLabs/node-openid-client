...
<form method="post" action="${this.issuer.authorization_endpoint}">
  ...
  async oauthCallback(
    ...
    if (params.code) {
       ...
       * START OF HACK
       * 
       * Big old hack. The issue here is that for some folks (Ian and Salim) Facebook is returning BOTH code and idToken
       * The scenario is mentioned here https://developers.facebook.com/docs/facebook-login/manually-build-a-login-flow/
       * But nothing is said about how to fix it.
       */

       let facebookLogin = false;
       console.log("RUNNING FROM OUR MODULE, YARRR");
      
       if (typeof(redirectUri) == "string") {
         facebookLogin = redirectUri.includes('/api/auth/callback/facebook');
       }
 
       if (facebookLogin) {
         console.log("This is Facebook, will not do id_token extended check");
       }
 
       if ('id_token' in tokenset && !facebookLogin) {
         throw new RPError({
           message:
             'id_token detected in the response, you must use client.callback() instead of client.oauthCallback()',
           params,
