...
console.log("using open-id 2023/10/17 lw v2");
...
<form method="post" action="${this.issuer.authorization_endpoint}">
  ...
  async oauthCallback(
    ...
    if (typeof params.id_token === 'string' && params.id_token.length && !facebookLogin) {
      throw new RPError({
        message:
    ...
    if (params.code) {
      ...
      if (typeof tokenset.id_token === 'string' && tokenset.id_token.length && !facebookLogin) {
        throw new RPError({
          message:
            'id_token detected in the response, you must use client.callback() instead of client.oauthCallback()',
          params,
